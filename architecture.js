document.addEventListener('DOMContentLoaded', function() {
    const apiKey = 't5jEcIX6LU1fE_TK2Ei4zJSsioBRU4oyHPiR8ODkYCU'; // Replace with your API key
    const apiUrl = `https://api.unsplash.com/photos/random?query=nature&count=20&client_id=${apiKey}`;

    fetch(apiUrl)
        .then(response => response.json())
        .then(data => {
            const gallery = document.getElementById('architecture-gallery');
            data.forEach(image => {
                ...
                overlay.innerHTML = `
                    <h3>${image.description || 'architecture'}</h3>
                    <p>${image.alt_description || 'Beautiful architecture photography.'}</p>
